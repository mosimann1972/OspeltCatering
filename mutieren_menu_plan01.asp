...
if Request.QueryString("M") = "Lo" then
	...
	response.Redirect "notfound.asp?Meldung=Menüplan kann nicht gelöscht werden!"	
...
SQL = "Select * from tbMenuPlan Order by Jahr Desc, KW Desc, MenuArtId"
RS.Open SQL,conn,3,3
...
if FirstFlag = true then
    ...
    %>
        ...
        <tr>
            ...
            <td><B><% = getMenuArt(RS("MenuArtId")) %></B></td>
            <td><a href="mutieren_menu_plan02.asp?Jahr=<% = RS("Jahr") %>&Kw=<% = RS("KW") %>&MenuArtId=<% = RS("MenuArtId") %>&M=Mu">mutieren</a></td>
