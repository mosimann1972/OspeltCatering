...
<B>Suppe mutieren/löschen</B> <% if AdminFlag = true then %><font color="red"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<% = getRestaurantName(UserId) %></b></font><% end if %>
<hr size="1">
...
<% do while not RS.eof and Counter < 50 %>
    ...
    <td><a href="mutieren_bestellung_suppe02.asp?Jahr=<% = Jahr %>&KW=<% = RS("KW") %>&Menu=<% = RS("Menu") %>&BestellUebersichtId=<% = RS("BestellUebersichtId") %>&M=Mu">mutieren</a></td>
    <td><a href="anzeigen_bestellung_suppe01.asp?Jahr=<% = Jahr %>&KW=<% = RS("KW") %>&Menu=<% = RS("Menu") %>&BestellUebersichtId=<% = RS("BestellUebersichtId") %>&M=Az">anzeigen</a></td>
    <td><a onclick="return confirm('Wirklich loeschen?')"  href="mutieren_bestellung_suppe01.asp?Jahr=<% = Jahr %>&KW=<% = RS("KW") %>&UserId=<% = UserId %>&BestellUebersichtId=<% = RS("BestellUebersichtId") %>&M=Lo">löschen</a></td>
