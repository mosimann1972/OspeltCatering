...
<B>Menü mutieren</B> <% if AdminFlag = true then %><font color="red"><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<% = getRestaurantName(RS("UserId")) %></b></font><% end if %>
<hr size="1">
<br />
<% = getKW(Kw,Jahr) %>
...
<form method="POST" action="mutieren_bestellung_menu03.asp">
...
<tr>
    ...
    <td>
        ...
        <select name="txtSB2PlusMinus<% = RS("Tag") %>">
            <option><% = RS("PM_SB2") %></option>   
            <option></option>
            <option>+</option>
            <option>-</option>
        </select>
        ...
        p = instr(1,RS("SB2"),"-") 
        if p <> 0 then SB2 = mid(RS("SB2"),2) else SB2 = RS("SB2") end if
        p = 0
        
        %>

        <input type="text" value=<% = SB2 %> name="txtSB2<% = RS("Tag") %>" size="5">
        
        
        </td>
        <td><font size="1"><% = getMenuName(RS("MenuId")) %></font></td>
</tr>    
